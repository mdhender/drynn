...
type pointGenerator interface {
	Generate(n int, r *rand.Rand) []point
}
...
// nearestNeighbor returns the index of the point in set closest to set[p]
...
func nearestNeighbor(set []point, p int) int {
	best := -1
	...
	for i, q := range set {
		...
		if d := distanceSquared(set[p], q); d < bestDistSq {
			bestDistSq = d
			best = i
		}
	}
...
// noCloserThan reports whether every point in set other than set[p] lies
...
func noCloserThan(set []point, p int, d float64) bool {
	if d <= 0 {
		return true
	}
	dSq := d * d
	for i, q := range set {
		...
		if distanceSquared(set[p], q) < dSq {
			return false
		}
	}
	return true
...
func (pg naiveDiskPointsGenerator) Generate(n int, r *rand.Rand) []point {
...
func (pg uniformDiskPointsGenerator) Generate(n int, r *rand.Rand) []point {
	if n <= 0 {
		return nil
	}
	points := make([]point, 0, n)
